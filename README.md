...
| **"Run Claude Code" step fails:** *"Environment variable validation failed: Either ANTHROPIC_API_KEY or CLAUDE_CODE_OAUTH_TOKEN is required when using direct Anthropic API."* | The action validates `ANTHROPIC_API_KEY` as an env var (not its input) before invoking Claude Code. The reusable workflow now sets a placeholder when proxying. | Already handled by `claude-fix.yml` — if you're seeing this, your central repo is on a stale `@main`. Re-pull the central repo or pin the caller to the latest commit. |
