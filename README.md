...
1. **Tag a release** on this central repo: `git tag v1 && git push --tags`. Have consuming repos pin to the tag: `uses: my-org/sonar-fix/.github/workflows/fix.yml@v1`.
2. **Copy the caller workflow** (`examples/caller-comment-triggered.yml`) to each new repo and edit `my-org`.
