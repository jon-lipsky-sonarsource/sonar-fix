...
2. **Triage** the issues against your repo's autofix config (which severities, types, and rules to fix vs. flag)
...
│   sonar-autofix.yml │──── calls ───▶│   path2-claude-autofix   │
│                     │               │   path3-copilot-autofix  │
│ .github/            │               │                          │
│   sonar-autofix-    │               │ triage-action/           │
...
Create a repo in your org called `sonar-autofix` (or any name). Push the contents of this repository to it. Under **Settings > Actions > General**, set "Access" to allow other repos in the org to use workflows and actions from this repo.
...
The workflow will trigger on `pull_request` events. Check the Actions tab to see the scan, triage, and auto-fix jobs.
...
- Customize `sonar-autofix-config.yml` per-repo as needed (different rules, severities, etc.)
...
**File:** `.github/workflows/path2-claude-autofix.yml`
...
| `config-path`       | No       | `.github/sonar-autofix-config.yml` | Path to autofix config |
...
**File:** `.github/workflows/path3-copilot-autofix.yml`
...
Each consuming repo has its own `.github/sonar-autofix-config.yml` that controls:

- **`agent`** — `claude`, `copilot`, or `both`
- **`auto_fix.severities`** — Which severities to auto-fix
- **`auto_fix.types`** — Which issue types (BUG, CODE_SMELL, etc.)
- **`auto_fix.rules.allow`** — Rules always auto-fixed
- **`auto_fix.rules.deny`** — Rules never auto-fixed (security-sensitive, etc.)
- **`paths.exclude`** — File globs to skip
- **`guardrails.max_issues_per_run`** — Cost cap
- **`guardrails.max_turns`** — Agent iteration cap

See `examples/sonar-autofix-config.yml` for a fully annotated starter config.
...
```yaml
uses: my-org/sonar-autofix/.github/workflows/path2-claude-autofix.yml@v1
```
