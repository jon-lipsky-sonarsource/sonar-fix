...
| **Workflow finishes `success` (`is_error: false`), agent burned 10–15 turns, but no commit appears on the PR.** | Agent committed locally but never ran `git push`. The runner is ephemeral, so unpushed commits are lost. Common when the agent's instructions say "commit" but not "push". | The current `prompts/sonar-fix-agent.md` explicitly tells the agent to `git push origin HEAD` after the commit. If you've forked the central repo and customized that prompt, restore the COMMIT AND PUSH section. To diagnose a future occurrence, set `vars.SHOW_FULL_OUTPUT=true` and look at the agent's Bash tool calls — if you see `git commit` but no `git push`, this is the cause. |
