...
4. **Repeat Step 5** for each new repo — MCP config, Copilot environment secrets, and the workflow-approval flip are all per-repo.
5. **(Optional) Override the central config** per repo by creating `.github/sonar-fix-config.yml`.
